...
from analyze_with_stockfish import STOCKFISH_PATH, make_logger
...
def pick_unseen_players(speed_category: str, n_players: int, already_used: set, seed: int) -> list:
    ...
    # sorted(), not just list(...): Python randomizes string hashing per
    # process (for security), so a set's iteration order -- and therefore
    # list(some_set) -- differs between runs even with identical contents.
    # Without sorting first, the SAME --seed would still shuffle a
    # differently-ordered starting list each time and produce different
    # candidates, defeating the point of passing a seed at all.
    candidates = sorted(eligible - already_used)
...
def main():
    ...
    log, log_path = make_logger(prefix="spot_check")
    ...
    log(f"Trying up to {len(candidates)} candidate players to find {args.n_players} usable ones...\n")
    ...
    try:
        for username in candidates:
            ...
            try:
                ...
                log(f"  [skip] {username}: {error}")
                continue
            if not pgn_text.strip():
                log(f"  [skip] {username}: no games returned")
                continue

            games = parse_games_for_player(pgn_text, username)
            if len(games) < MIN_GAMES_REQUIRED:
                log(f"  [skip] {username}: only {len(games)} games")
                continue
            ...
            log(f"  {username}: actual={actual:.0f}  predicted={predicted:.0f}  error={error:.0f}")
    finally:
        engine.quit()
    ...
    log(f"Mean absolute error:   {df['abs_error'].mean():.1f}")
    log(f"Median absolute error: {df['abs_error'].median():.1f}")
    log(f"(for reference, the formal held-out test MAE was {bundle['test_mae']:.1f})")
